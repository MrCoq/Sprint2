...
function Utilisateur_Changer_Status_ParId($connexionPDO, $idUtilisateur, $statusUtilisateur)
{
    $requetePreparée = $connexionPDO->prepare(
        'UPDATE `utilisateur` 
    SET `statusUtilisateur`= :paramStatus
    WHERE idUtilisateur = :paramidEntreprise');
    $requetePreparée->bindParam('paramidEntreprise', $idUtilisateur);
