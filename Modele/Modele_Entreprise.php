...
function Entreprise_Changer_Status_ParId($connexionPDO, $idEntreprise, $statusCompte)
{
    $requetePreparée = $connexionPDO->prepare(
        'UPDATE `entreprise` 
    SET `statusCompte`= :paramStatus
    WHERE idEntreprise = :paramidEntreprise');
    $requetePreparée->bindParam('paramidEntreprise', $idEntreprise);
    $requetePreparée->bindParam('paramStatus', $statusCompte);
